(function (global, factory) {
      ...
      if (innerPoints.length > 1) {
          ...
          if (disjoint) {
              ...
              arcs.push({circle : smallest,
                         ...
                         p2: { x: smallest.x - SMALL$2, y : smallest.y + smallest.radius},
  ...
  function containedInCircles(point, circles) {
      for (var i = 0; i < circles.length; ++i) {
          if (distance(point, circles[i]) > circles[i].radius + SMALL$2) {
              return false;
          }
      }
      return true;
  ...
  function venn() {
    ...
    to sets = [
        {sets: ['A'], size: 3, nodes : ['node_0', 'node_2', 'node_3']},
        {sets: ['B'], size: 3, nodes : ['node_1', 'node_2', 'node_3']},
    ...
    */
    function extractSets(data) {
      ...
      function powerset(set) {
        ...
        var front = [set.shift()];
        var pset = powerset(set);
        return pset.concat(pset.map(function(d) {
          return front.concat(d);
        }));
      }
      ...
      for (i = -1; ++i < n;) {
        set = accessor(data[i]);
        if (set.length) {
          data[i].__setKey__ = set.sort().join(',');

          psets = powerset(set).slice(1);
          psets.forEach(function (pset) {
            key = pset.sort().join(',');
            if (s = sets.get(key)) {
              s.size += 1;
              s.nodes.push(data[i]);
